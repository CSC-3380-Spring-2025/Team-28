...
const Sidebar = React.forwardRef((_a, ref) => {
    ...
    return (<div ref={ref} className="group peer hidden text-sidebar-foreground md:block" data-state={state} data-collapsible={state === "collapsed" ? collapsible : ""} data-variant={variant} data-side={side}>
        ...
        <div className={(0, utils_1.cn)("relative w-[--sidebar-width] bg-sidebar transition-[width] duration-200 ease-linear", "group-data-[collapsible=offcanvas]:w-0", "group-data-[side=right]:rotate-180", variant === "floating" || variant === "inset"
...
const SidebarTrigger = React.forwardRef((_a, ref) => {
    ...
    return (<button_1.Button ref={ref} data-sidebar="trigger" variant="ghost" size="icon" className={(0, utils_1.cn)("w-sm h-screen flex items-start p-5 justify-center bg-sidebar", className)} onClick={(event) => {
            onClick === null || onClick === void 0 ? void 0 : onClick(event);
            toggleSidebar();
        }} aria-label="Toggle Sidebar" {...props}>
      <lucide_react_1.AlignJustify className="text-white"/>
...
const SidebarFooter = React.forwardRef((_a, ref) => {
    var { className } = _a, props = __rest(_a, ["className"]);
    return (<div ref={ref} data-sidebar="footer" className={(0, utils_1.cn)("flex flex-col gap-2", className)} {...props}/>);
});
